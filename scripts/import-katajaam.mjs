...
Existing listings are never overwritten, republished, or deleted.`)
  ...
  const inventory = o.input ? JSON.parse(readFileSync(o.input,'utf8')) : await crawl(readPublic, { checkpoint: inventory => writeFileSync(join(dir,'inventory.partial.json'), JSON.stringify(inventory,null,2)) })
  if (inventory.complete === false) throw Error('Inventory is incomplete; refusing import')
